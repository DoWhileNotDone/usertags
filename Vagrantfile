...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "shell", name: "install nvm and node", privileged: false, inline: <<-SHELL
    export DEBIAN_FRONTEND=noninteractive
    cd && curl -o- https://raw.githubusercontent.com/creationix/nvm/v0.34.0/install.sh | bash
  ...
  config.vm.provision "shell", name: "configure apache", inline: <<-'SHELL'
    ...
    ln -s /vagrant/node_practice/public /var/www/html/node
    ln -s /vagrant/sampleservice/public /var/www/html/sample
  ...
  config.vm.provision "shell", name: "configure postgres", inline: <<-SHELL
    sed -i '/# IPv4 local connections:/ a host    all             postgres        samehost                trust' /etc/postgresql/11/main/pg_hba.conf
    sed -i -e 's/local   all             postgres                                peer/local   all             postgres                                trust/' /etc/postgresql/11/main/pg_hba.conf
